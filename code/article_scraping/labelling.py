        ...
        for i in range(0, len(sentences)):
            ...
            for j in range(0, len(article_sentences), chunk_size):
                ...
                if len(output_split) != len(chunk):
                    print(f"At index {i}, the article {article_topics[i]}, chunk {j // chunk_size + 1}, the number of labels is {len(output_split)}, while the number of sentences in the chunk is {len(chunk)}, check the output.")
                    break

                total_output.extend(output_split)

            if len(total_output) != len(article_sentences):
                print(f"At index {i}, the article {article_topics[i]}, the number of labels is {len(total_output)}, while the number of sentences in the article is {len(article_sentences)}, check the output.")
                break
