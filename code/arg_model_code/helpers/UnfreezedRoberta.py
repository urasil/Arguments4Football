...
# Custom configuration for RoBERTaEnhanced
class RoBERTaEnhancedConfig(RobertaConfig):
...
class RoBERTaEnhanced(PreTrainedModel):
    config_class = RoBERTaEnhancedConfig
    ...
    def __init__(self, config):
        ...
        # RNN layer
        self.rnn = torch.nn.GRU(combined_dim, 768, batch_first=True, num_layers=config.num_rnn_layers, bidirectional=True)
        ...
        self.post_init()  # Ensures the model is initialized correctly
    ...
    def forward(self, input_ids, attention_mask, features, labels=None):
        ...
        # Concatenate CLS token output with additional features
